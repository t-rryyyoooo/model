...
class UNetGenerator(nn.Module):
    def __init__(self, input_ch, output_ch, num_down, ngf=64, norm_layer=nn.BatchNorm3d, use_dropout=False):
...
class UNetSkipConnectionBlock(nn.Module):
    ...
    def __init__(self, outer_ch, inner_ch, input_ch=None, submodule=None, outermost=False, innermost=False, norm_layer=nn.BatchNorm3d, use_dropout=False):
        ...
        down_conv = nn.Conv3d(input_ch, inner_ch, kernel_size=4, stride=2, padding=1, bias=use_bias)# Half the image size.
        ...
        if outermost:
            up_conv = nn.ConvTranspose3d(inner_ch * 2, outer_ch, kernel_size=4, stride=2, padding=1, bias=use_bias)# Double the image size.
            down = [down_conv]
            up = [up_relu, up_conv, nn.Tanh()]
            model = down + [submodule] + up

        elif innermost:
            up_conv = nn.ConvTranspose3d(inner_ch, outer_ch, kernel_size=4, stride=2, padding=1, bias=use_bias)
...
if __name__ == "__main__":
    ...
    x = torch.Tensor(1, input_ch, 2**num_down, 2**num_down, 2**num_down) 
    print(x.size())
    pred = ug.forward(x)
    print(pred.size())
