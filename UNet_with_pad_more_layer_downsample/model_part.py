...
class CreateConvBlock(nn.Module):
    def __init__(self, in_channel, mid_channel, out_channel, n=2, use_bn=True, apply_pooling=True, pooling_size=(2, 2, 2)):
        ...
        if apply_pooling:
            self.maxpool = nn.MaxPool3d(pooling_size)
...
class CreateUpConvBlock(nn.Module):
    def __init__(self, in_channel, concat_channel, mid_channel, out_channel,  n=2, use_bn=True, upsampling_size=(2, 2, 2)):
        super(CreateUpConvBlock, self).__init__()

        x = nn.ConvTranspose3d(in_channel, in_channel, upsampling_size, stride=upsampling_size, padding=(0, 0, 0), dilation=1)
