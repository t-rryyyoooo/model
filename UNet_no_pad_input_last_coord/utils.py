...
def separateData(image_path, coord_first_path, coord_last_path, criteria, phase): 
    dataset = []
    for number in criteria[phase]:
        ip = Path(image_path) / ("case_" + number) 
        cfp = Path(coord_first_path) / ("case_" + number) 
        clp = Path(coord_last_path) / ("case_" + number) 

        image_list = ip.glob("image*")
        label_list = ip.glob("label*")
        feature_first_list = cfp.glob("coordinate*")
        feature_last_list = clp.glob("coordinate*")
        
        image_list = sorted(image_list)
        label_list = sorted(label_list)
        feature_first_list = sorted(feature_first_list)
        feature_last_list = sorted(feature_last_list)

        assert len(image_list) == len(feature_first_list) == len(label_list) == len(feature_last_list)
        for img, fea_first, fea_last, lab in zip(image_list, feature_first_list, feature_last_list, label_list):
            dataset.append(((str(img), str(fea_first), str(fea_last)), str(lab)))
